...
mod tests {
    ...
    fn rotate_rejects_same_epoch_id() {
        ...
        match err {
            Error::Storage(msg) => assert!(msg.to_string().contains("matches current epoch id"), "got {msg}"),
    ...
    fn rotate_rejects_already_retired_epoch_id() {
        ...
        match err {
            Error::Storage(msg) => assert!(msg.to_string().contains("already retired"), "got {msg}"),
            other => panic!("expected Storage error, got {other:?}"),
        }
    }
